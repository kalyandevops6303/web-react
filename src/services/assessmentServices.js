...
const addAssessmentService = ({ assessmentName, assessmentId, strType, id }) => DataService.put(`${API.assessments.add_assessment}?assessment_name=${assessmentName}&assessment_id=${assessmentId}&str_type=${strType}&_id=${id}`);
const assessmentLinkService = ({ assessmentName, assessmentId }) => DataService.put(`${API.assessments.get_assessment_link}?assessment_name=${assessmentName}&assessment_id=${assessmentId}`);
const deleteNonAssessmentService = ({ strType, id }) => DataService.delete(`${API.assessments.delete_non_assessment}?str_type=${strType}&_id=${id}`);
