...
const inviteDelegate =
  ({ email, onSuccess }) =>
    async (dispatch) => {
      ...
      try {
        await inviteDelegateService({ email, invitation_type: "FULL_ACCESS" });
...
const invitePaymentDelegate =
  ({ email, onSuccess }) =>
    async (dispatch) => {
      dispatch(inviteDelegateRequest());
      try {
        await inviteDelegateService({ email, invitation_type: "PAYMENT_DELEGATE" });
        dispatch(inviteDelegateSuccess());
        if (onSuccess) {
          onSuccess();
        }
