...
import { CometChatAvatar, CometChatConfirmDialog, CometChatUserPresence } from '../../Shared';
...
class CometChatViewGroupMemberListItem extends React.Component {
  ...
  render() {
    ...
    let title = this.props.member.metadata?.title || '';
        ...
        <div css={modalRowStyle(this.context)} className="content__row">
          <div css={nameColumnStyle(this.context, editClassName)} className="userinfo">
            ...
            <div
              ...
              onMouseLeave={(event) => this.toggleTooltip(event, false)}
            >
              <div css={namePStyle()}>{name}</div>
              <div css={desStyle()}>{title}</div>
            </div>
          </div>
          ...
          {this.props.member.scope === CometChat.GROUP_MEMBER_SCOPE.ADMIN ? (
            <div css={adminTagStyle()} className="admin__tag">
              Admin
            </div>
          ) : (
            editAccess
