    ...
    this.AddMembersManager.fetchNextUsers()
      .then((userList) => {
        ...
        if (filteredUserList.length === 0) {
          this.setState({
            decoratorMessage: Translator.translate('NO_USERS_FOUND', this.context.language),
          });
        }
      ...
      .catch((error) =>
        this.setState({
          decoratorMessage: Translator.translate('SOMETHING_WRONG', this.context.language),
        }),
      );
  };
  ...
  updateMembers = () => {
    ...
    if (membersList.length) {
      ...
      CometChat.addMembersToGroup(guid, membersList, [])
        ...
        .catch((error) => {
          console.log(error);
          let message = 'SOMETHING_WRONG';
          if (error.code === 'ERR_GROUP_NO_ADMIN_SCOPE') message = 'You need admin access to add participant';
          this.setState({
            addingMembers: false,
            errorMessage: Translator.translate(message, this.context.language),
          });
        });
    }
  };
    ...
    if (this.state.filteredlist.length === 0) {
      messageContainer = (
        <div css={contactMsgStyle()} className="members__decorator-message">
          <p css={contactMsgTxtStyle(this.context)} className="decorator-message">
