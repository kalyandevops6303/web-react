...
class CometChatGroupList extends React.PureComponent {
      ...
      .then((groupList) => {
        if (groupList.length === 0) {
          if (this.state.grouplist.length === 0) {
            this.setState({
              decoratorMessage: Translator.translate('NO_GROUPS_FOUND', this.props.lang),
            });
          }
        } else {
          this.setState({
  ...
  render() {
    ...
    const groups = this.state.grouplist.map((group) => {
      let selectedGroup =
        this.getContext().type === CometChat.ACTION_TYPE.TYPE_GROUP && this.getContext().item.guid === group.guid
          ? group
          : null;
