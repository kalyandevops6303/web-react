...
class CometChatGroupDetails extends React.Component {
  ...
  render() {
    ...
    if (this.context.item.scope === CometChat.GROUP_MEMBER_SCOPE.ADMIN) {
      ...
      deleteGroupBtn = (
        <div css={contentItemStyle()} className="content__item">
          <span css={itemLinkStyle(this.context, 1)} className="item__link">
            <button css={closeBtn()} onClick={this.props.closePopup}>
              Close
            </button>
            <button css={delGroupBtn()} onClick={this.deleteGroup}>
              {Translator.translate('Delete Group', this.context.language)}
            </button>
          </span>
        </div>
      );
    ...
    if (this.props.addMember) {
      addMembers = (
        <CometChatAddGroupMemberList
          close={() => {
            this.clickHandler('addmember', false);
            this.props.setAddMember();
          }}
          actionGenerated={this.membersActionHandler}
        />
      );
    ...
    if (this.state.showDeleteConfirmDialog) {
      showDeleteConfirmDialog = (
        <CometChatConfirmDialog
          {...this.props}
          title="Delete Group"
          description="Are you sure you want to delete the group permanently?"
    ...
    if (this.state.showTransferOwnershipConfirmDialog) {
      showTransferOwnershipConfirmDialog = (
        <CometChatConfirmDialog
          ...
          description="Are you sure you want to transfer the ownership?"
