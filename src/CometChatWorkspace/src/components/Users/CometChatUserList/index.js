  ...
  render() {
    ...
    const users = userList.map((user) => {
      ...
      if (chr !== currentLetter) {
        currentLetter = chr;
        firstChar = (
          <div css={contactAlphabetStyle(this.props)} className="contacts__list__alphabet-filter">
            {currentLetter}
          </div>
        );
      } else {
      ...
      let selectedUser =
        this.getContext().type === CometChat.ACTION_TYPE.TYPE_USER && this.getContext().item.uid === user.uid
          ? user
          : null;
      return (
        <React.Fragment key={user.uid}>
          ...
          <CometChatUserListItem
            type={this.props.type ? 'group' : 'direct'}
            user={user}
    ...
    if (this.state.enableSearchUser) {
      searchUser = (
        <div css={contactSearchStyle(this.props)} className="contacts__search">
          ...
          <input
            ...
            placeholder={Translator.translate('Search member name', this.props.lang)}
      ...
      <div css={contactWrapperStyle(this.props, theme)} className="contacts">
        {this.props.type !== 'group' ? (
          <div css={closeBtnContainer()}>
            <img onClick={this.props.close} css={topPopUpCloseBtn()} src={closeIcon} />
          </div>
        ) : null}
        <div css={contactHeaderStyle(this.props, theme)} className="contacts__header">
          ...
          >
            {this.props.type !== 'group'
              ? Translator.translate('Send Direct Message', this.props.lang)
              : Translator.translate('SELECT GROUP MEMBERS', this.props.lang)}
          </p>
        </div>
        {searchUser}
        {messageContainer}
        ...
        {/* <hr css={endLine()} /> */}
        {/* {this.props.type!=="group"?<div css={btnContainer()}>
				<button css={closeButton()}>
					Close
				</button>
				<button css={sendMessageBtn()}>Send Message</button>
				</div>:null} */}
      </div>
    );
