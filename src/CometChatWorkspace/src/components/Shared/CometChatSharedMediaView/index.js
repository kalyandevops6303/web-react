...
import download from '../../../../../assets/images/chat/download.png';
import {
  sectionStyle,
  sectionHeaderStyle,
  sectionContentStyle,
  ...
  render() {
    const template = (message, key) => {
      if (this.state.messagetype === 'image' && message?.data?.attachments) {
        let img = new Image();
        img.src = message?.data?.attachments[0].url;
        ...
        return (
          ...
          >
            <img src={message.data.attachments[0].url} alt={Translator.translate('SHARED_MEDIA', this.props.lang)} />
          </div>
        );
        ...
        return (
          ...
          >
            <div>{message.data.attachments[0].name}</div>
            <div>
    ...
    return (
      <div css={sectionStyle(this.props)} className="section section__sharedmedia">
        {/* {this.state.full?<CometChatImageViewer message={this.state.data} />:null}
        ...
        <div css={sectionContentStyle(this.props)} data-id="sharedmedia" className="section__content">
          <div css={mediaBtnStyle()} className="media__button">
            <span css={buttonStyle(this.state, 'image')} onClick={() => this.mediaClickHandler('image')}>
              {Translator.translate('PHOTOS', this.props.lang)}
            </span>
            <span css={buttonStyle(this.state, 'video')} onClick={() => this.mediaClickHandler('video')}>
              {Translator.translate('VIDEOS', this.props.lang)}
            </span>
            <span css={buttonStyle(this.state, 'file')} onClick={() => this.mediaClickHandler('file')}>
