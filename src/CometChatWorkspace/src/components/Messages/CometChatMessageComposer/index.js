...
import videoIcon from './resources/video-file.svg';
...
class CometChatMessageComposer extends React.PureComponent {
  ...
  componentDidMount() {
    ...
    this.item =
      this.context.type === CometChat.ACTION_TYPE.TYPE_USER || this.context.type === CometChat.ACTION_TYPE.TYPE_GROUP
        ? this.context.item
        : null;
  ...
  sendTransientMessage = () => {
    ...
    const receiverId =
      this?.context?.type === CometChat.ACTION_TYPE.TYPE_USER ? this?.context?.item?.uid : this?.context?.item?.guid;
  ...
  render() {
    ...
    let docs = (
      <div
        title={docText}
        css={fileItemStyle(attachIcon, this.context)}
    ...
    if (this.state.emojiViewer) {
      emojiViewer = (
        <CometChatEmojiKeyboard
          ...
          onClose={() => this.setState({ ...this.state, emojiViewer: false })}
        />
      );
    }
