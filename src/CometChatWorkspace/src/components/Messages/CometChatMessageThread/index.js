...
class CometChatMessageThread extends React.PureComponent {
  ...
  getSenderMessageComponent = (message) => {
    ...
    switch (message.type) {
      ...
      case CometChat.MESSAGE_TYPE.IMAGE:
        component = (
          <CometChatSenderImageMessageBubble key={message.id} message={message} actionGenerated={this.actionHandler} />
        );
        break;
      case CometChat.MESSAGE_TYPE.FILE:
        component = (
          <CometChatSenderFileMessageBubble key={message.id} message={message} actionGenerated={this.actionHandler} />
        );
        break;
      case CometChat.MESSAGE_TYPE.VIDEO:
        component = (
          <CometChatSenderVideoMessageBubble key={message.id} message={message} actionGenerated={this.actionHandler} />
        );
        break;
      case CometChat.MESSAGE_TYPE.AUDIO:
        component = (
          <CometChatSenderAudioMessageBubble key={message.id} message={message} actionGenerated={this.actionHandler} />
        );
        break;
      default:
        break;
  ...
  getReceiverMessageComponent = (message) => {
    ...
    switch (message.type) {
      ...
      case CometChat.MESSAGE_TYPE.FILE:
        component = (
          <CometChatReceiverFileMessageBubble key={message.id} message={message} actionGenerated={this.actionHandler} />
        );
        break;
      case CometChat.MESSAGE_TYPE.AUDIO:
        component = (
  ...
  render() {
    ...
    if (this.state.parentMessage.hasOwnProperty('replyCount')) {
      ...
      const replyText =
        replyCount === 1
          ? `${replyCount} ${Translator.translate('REPLY', this.context.language)}`
          : `${replyCount} ${Translator.translate('REPLIES', this.context.language)}`;
    ...
    if (
      (this.context.type === CometChat.ACTION_TYPE.TYPE_USER && this.state.enableSendingOneOnOneMessage === false) ||
