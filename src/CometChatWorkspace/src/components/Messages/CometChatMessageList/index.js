...
class CometChatMessageList extends React.PureComponent {
  ...
  componentDidMount() {
    ...
    this.item =
      this.context.type === CometChat.ACTION_TYPE.TYPE_USER || CometChat.ACTION_TYPE.TYPE_GROUP
        ? this.context.item
        : {};
  ...
  componentDidUpdate(prevProps, prevState) {
    const ifChatWindowChanged = () => {
      ...
      if (
        ...
        (this.context.item.uid !== this.item.uid || this.context.item.blockedByMe !== this.item.blockedByMe)
      ) {
        output = true;
  ...
  onMessageReceived = (message) => {
    ...
    if (
      ...
      if (
        ...
        (message.getSender().uid === this.state.loggedInUser?.uid && message.getReceiverId() === this.context.item.uid)
      ) {
