      ...
      CometChat.getConversation(this.context.item.guid, 'group').then(
        (conversation) => {
          if (
            ...
            this.state.messageList.slice(-1)[0].id != conversation.lastMessage.id
          ) {
            this.setState({
              ...this.state,
    ...
    */
    ...
    let messageComponent = (
      <React.Fragment>
        ...
        >
          <CometChatMessageHeader
            sendData={({ data1, data2 }) => this.props.data({ avatar: data1, presence: data2 })}
            showPopup={this.props.onPopup}
            lang={this.props.lang}
