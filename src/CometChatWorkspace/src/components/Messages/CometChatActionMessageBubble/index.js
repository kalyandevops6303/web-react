    ...
    switch (call.status) {
      case CometChat.CALL_STATUS.INITIATED: {
        ...
        if (call.type === CometChat.CALL_TYPE.AUDIO) {
          if (call.receiverType === CometChat.RECEIVER_TYPE.USER) {
            actionMessage =
              call.callInitiator.uid === this.loggedInUser?.uid
                ? Translator.translate('OUTGOING_AUDIO_CALL', this.context.language)
                : Translator.translate('INCOMING_AUDIO_CALL', this.context.language);
            ...
            if (call.action === CometChat.CALL_STATUS.INITIATED) {
              ...
              actionMessage =
                call.sender.uid === this.loggedInUser?.uid
                  ...
                  : `${call.sender.name} ${Translator.translate('REJECTED_CALL', this.context.language)}`;
            }
          }
          ...
          if (call.receiverType === CometChat.RECEIVER_TYPE.USER) {
            actionMessage =
              call.callInitiator.uid === this.loggedInUser?.uid
                ? Translator.translate('OUTGOING_VIDEO_CALL', this.context.language)
                : Translator.translate('INCOMING_VIDEO_CALL', this.context.language);
          } else if (call.receiverType === CometChat.RECEIVER_TYPE.GROUP) {
            if (call.action === CometChat.CALL_STATUS.INITIATED) {
              actionMessage =
                call.callInitiator.uid === this.loggedInUser?.uid
          ...
          if (call.action === CometChat.CALL_STATUS.ONGOING) {
            actionMessage =
              call.sender.uid === this.loggedInUser?.uid
                ...
                : `${call.sender.name} ${Translator.translate('JOINED', this.context.language)}`;
          } else if (call.action === CometChat.CALL_STATUS.REJECTED) {
            actionMessage =
              call.sender.uid === this.loggedInUser?.uid
                ? Translator.translate('CALL_REJECTED', this.context.language)
            ...
            if (call.sender.uid === this.loggedInUser?.uid) {
              actionMessage = `${Translator.translate('YOU', this.context.language)} ${Translator.translate(
                'LEFT_THE_CALL',
      ...
      case CometChat.CALL_STATUS.UNANSWERED: {
        ...
        if (
          ...
          (call.receiverType === CometChat.RECEIVER_TYPE.USER || call.receiverType === CometChat.RECEIVER_TYPE.GROUP)
        ) {
          actionMessage =
            call.callInitiator.uid === this.loggedInUser?.uid
              ? Translator.translate('UNANSWERED_AUDIO_CALL', this.context.language)
              : Translator.translate('MISSED_AUDIO_CALL', this.context.language);
        } else if (
          ...
          actionMessage =
            call.callInitiator.uid === this.loggedInUser?.uid
              ? Translator.translate('UNANSWERED_VIDEO_CALL', this.context.language)
              : Translator.translate('MISSED_VIDEO_CALL', this.context.language);
        }
        break;
      }
