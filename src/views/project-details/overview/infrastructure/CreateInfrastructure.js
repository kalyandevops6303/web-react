...
const CreateInfrastructure = ({ updateInfraData }) => {
  ...
  return (
    <Form onSubmit={handleSubmit(onSubmit)}>
      <Row>
        <Col sm="9">
          <p className="fw-bold font-medium-5">No active infrastructure for this project.</p>
          <p>Add the services you need and submit to create the infrastructure.</p>
        </Col>
      </Row>
            ...
            <Row key={item.id} className="mt-1 d-flex align-items-center">
              <Col sm="12" md="12" lg="10">
                ...
                {errors?.servicesSelected?.[index]?.service && (
                  <FormFeedback>{errors.servicesSelected[index].service.label.message}</FormFeedback>
                )}
              </Col>
              {getValues(`servicesSelected.${index}.service.value`) === 'openai' && (
                <Col sm="12" md="12" lg="10">
                  ...
                  {errors?.servicesSelected?.[index]?.model && (
                    <FormFeedback>{errors.servicesSelected[index].model.label.message}</FormFeedback>
                  )}
                </Col>
              )}
              <Col sm="12" md="12" lg="2">
                {fields.length > 1 && (
                  <Button type="button" color="flat-danger" className="mt-2" onClick={() => handleRemoveService(index)}>
                    Remove
                  </Button>
                )}
              </Col>
