...
const ManageInfrastructure = ({ data, updateInfraData }) => {
  ...
  return (
    <div>
      <TerminateInfraModal
        modal={isTerminateInfraModal}
        toggleModal={() => setIsTerminateInfraModal(!isTerminateInfraModal)}
        terminateInfra={terminateInfra}
      />
      <Row className="my-1">
        <Col sm="12">
          <p>{getInfraDescriptionByStatus(data?.status)}</p>
        </Col>
      </Row>
      ...
      <div className="d-flex justify-content-end">
        ...
        {data?.status === InfraStatus.CREATED && (
          <Button
            ...
            outline
            className="me-2 px-3"
            onClick={() => setIsTerminateInfraModal(true)}
            disabled={isLoading}
          >
            Delete
          </Button>
