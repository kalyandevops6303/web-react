    ...
    <div>
      <div className="d-flex align-items-center justify-content-between">
        ...
        <AssessmentResultText grade={assessment.assessment_grade}>
          <b>
            <small>{assessment.assessment_grade}</small>
          </b>
        ...
        <CardHeader className="d-flex flex-column align-items-start w-100">
              ...
              <div className="d-flex gap-1">
                ...
                <div className="d-flex">
                  <div className="cursor-pointer" onClick={() => setShowAssessmentsList(!showAssessmentsList)}>
                    {showAssessmentsList ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
                  </div>
                </div>
              </div>
            )}
          ...
          <CardText className="mt-1">
            {userAssessmentsCount ? (
              <>
                <span>
                  {getAssessmentsLeftCount()} / {currentAssessmentLimit} Assessments Left
                </span>
              </>
            ) : (
            ...
            {showAssessmentsList && (
              <CardBody className="d-flex flex-column gap-1">
                {userAssessments?.map((assessment) => (
                  <>{assessment.completed_date && <AssessmentsListItem assessment={assessment} />}</>
                ))}
                <div className="d-flex justify-content-center">
                  <Link to="/assessments">
                    {getAssessmentsLeftCount(userAssessments) > 0 && (
                      <Button color="transparent" className="text-primary border-primary mt-0">
