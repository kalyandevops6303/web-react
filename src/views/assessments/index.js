...
import { NavigationText, AssessmentsNavigation, ArrowWrapper, NoteWrapper } from './style';
              ...
              <CardBody>
                <PaymentInfoBanner className="mb-2 d-flex px-1 py-2 align-items-center">
                  ...
                  <p className="font-medium-1 m-0 info d-flex align-items-center justify-content-between w-100">
                    <div>
                      <span className="fw-bolder font-medium-1">Note:</span> You are allowed up to{' '}
                      {currentAssessmentLimit} assessments. These assessment scores will increase your discoverability
                      to clients.
                    </div>
                    <NoteWrapper>
                ...
                <AssessmentsList
                  setOpen={location.state}
                  data={assessmentsListData}
                  dropdownOptions={dropdownList.filter(
                    (item) =>
                      !assessmentsListData?.some((assessment) => assessment.assessment_id === item.assessment_id),
                  )}
                />
