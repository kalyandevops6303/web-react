...
const AssessmentsRequestList = ({ notUserAssessments, supportRequests }) => {
  ...
  useEffect(() => {
    const totalEntries =
      (!isEmpty(notUserAssessments) ? notUserAssessments.length : 0) +
      (!isEmpty(supportRequests) ? supportRequests.length : 0);
    ...
    <>
      <div className="mt-5">
        <div className="d-flex align-items-center justify-content-between">
          {showTable && <CardTitle tag="h4">Skills & Tools Assessment Request &nbsp;</CardTitle>}

          <CardText className="d-flex align-items-center cursor-pointer" onClick={toggleCustomerSupportModal}>
            <Info size={18} color={theme.activeNavPillText} className="me-50 info-banner-icon" />
            <p className="font-medium-1 m-0 info text-primary">Couldn't find your skills assessment?</p>
          </CardText>
        </div>
