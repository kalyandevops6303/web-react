...
const AssessmentsList = ({ setOpen, data, dropdownOptions }) => {
  ...
  const handleAddAssessment = (value) => {
    if (value) {
      ...
      dispatch(
        addAssessment({
          assessmentName: value.value?.str_name,
          assessmentId: value.value?.assessment_id,
          strType: value.value?.str_type,
          id: value.value?._id,
        }),
      );
    }
  ...
  return (
    <>
      <div className="d-flex">
        <CardTitle tag="h4">Skills & Tools Assessment &nbsp;</CardTitle>
      </div>
      ...
      <TableWrapper>
        <Table responsive>
          ...
          <tbody>
            ...
            <td>
              <>
                {Array.from({ length: Math.max(currentAssessmentLimit - (data ? data.length : 0), 1) }).map(
                  (_, index) => (
                    <Select
                      isClearable
