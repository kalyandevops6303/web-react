...
export async function getPermittedFeatures(): Promise<Feature[]> {
...
export async function hasFeatureAccess(feature_name: string): Promise<boolean> {
  try {
    const features = await getPermittedFeatures();
