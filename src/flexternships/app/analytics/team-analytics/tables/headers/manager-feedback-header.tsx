...
export default function ManagerFeedbackHeader({ column, noWrap }: Readonly<{ column: Column<any>; noWrap?: boolean }>) {
  return (
    <Button
      variant="ghost"
      ...
      <span className="text-grey-heading font-montserrat text-xs font-semibold leading-none tracking-wider uppercase text-wrap">
        <div className="flex flex-col items-start justify-start gap-1">
          <div className="flex flex-row justify-start gap-2 text-left">
            <span>MANAGER {noWrap && 'FEEDBACK'}</span>
            {noWrap && <TooltipInfo>Manager Feedback</TooltipInfo>}
          </div>
          {!noWrap && (
