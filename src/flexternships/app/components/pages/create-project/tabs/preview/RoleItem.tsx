...
export default function RoleItem(props: Props) {
  ...
  return (
    <div className={`${Styles.roleItem} ${last ? '' : 'border-b-1'}`}>
      <div className={Styles.role}>{String(data.role?.name || '(Add role)')}</div>
      <div className={Styles.count}>{String(data.count || '(Add count)')}</div>
      <div className={Styles.skills}>
        {data.skills?.map((skill, index) => <PrimaryTag key={index} content={String(skill?.name || 'Skill')} />) || []}
      </div>
      <div className={Styles.tools}>
        {data.tools?.map((tool, index) => <PrimaryTag key={index} content={String(tool?.name || 'Tool')} />) || []}
      </div>
    </div>
  );
}
