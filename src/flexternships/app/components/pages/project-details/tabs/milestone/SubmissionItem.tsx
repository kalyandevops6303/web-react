...
export default function SubmissionItem(props: Props) {
    ...
    return (
        <div className={`flex flex-row min-h-[52px] items-center border-solid border-grey-c2 px-1.5 ${last ? '' : 'border-b-1'}`}>
            ...
            <div className='py-4 px-2.5 w-[194px]'>
                {formatEpochToHumanReadable(data.submittedAt, false, true)}
            </div>
            <div className='py-4 px-2.5 w-[122px] flex flex-row items-center gap-x-3'>
                <span className="flex items-center justify-center bg-trublue-light rounded-full p-2 text-trublue-secondary-500 cursor-pointer" onClick={handleMainActionClick}>
                    {
                        mainActionLoading ? (
