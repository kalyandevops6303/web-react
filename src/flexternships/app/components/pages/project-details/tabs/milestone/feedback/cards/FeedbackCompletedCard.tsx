...
export default function FeedbackCompletedCard(props: FeedbackCompletedCardProps) {
  ...
  return (
    <div
      className={`flex flex-row justify-between items-center px-6 py-5 bg-success bg-opacity-[0.12] rounded-md ${tiny && 'w-6/12'
        }`}
    >
      ...
      <div className='flex flex-row items-center gap-x-4'>
        <span className={`${tiny ? 'text-sm text-success font-semibold' : 'text-base text-trublue-secondary-500 font-medium'} not-italic cursor-pointer`}>
          View
        </span>
        {tiny && (
