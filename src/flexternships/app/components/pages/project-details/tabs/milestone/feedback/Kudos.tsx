...
export class KudosModel extends Question {
    ...
    onSurveyLoad() {
        if (this.jsonObj && this.jsonObj.choices) {
            const parsedChoices = this.jsonObj.choices.map(
                (choice) => new ItemValue(choice.value, choice.text)
            );
                    ...
                    {choices.length > 0 ? (
                        choices.map((choice: any, index: number) => (
                            <button
                                ...
                                className={`p-2 border rounded-lg w-24 justify-center flex items-center text-xs gap-2 font-sans ${selectedValue === choice.value
                            ...
                            >
                                {choice.text === "NA" && choice.text !== "kudos" ? (
                                    ...
                                    <ThumbsUp className={`${selectedValue === choice.value ? "text-skyblue" : "text-gray-400"
                                        } text-xs`} />
                                )}
                                {choice.text}
                            </button>
