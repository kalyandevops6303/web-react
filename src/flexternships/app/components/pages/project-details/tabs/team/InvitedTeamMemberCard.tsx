...
import { getMaskedEmail } from '@/flexternships/utils/text-utils';

export default function InvitedTeamMemberCard(props: InputProps) {
  const { member } = props;

  const displayName = (member.name || (member.email && getMaskedEmail(member.email))) ?? 'Unknown Name';
  return (
    <div className="p-5 flex flex-row flex-wrap justify-between items-center gap-y-6 rounded-md bg-white shadow-card">
      <div className="flex flex-row flex-wrap items-center gap-x-7 gap-y-6">
        <div className="flex items-center gap-x-3">
          ...
          <span className="text-grey text-sm font-semibold leading-5.5 w-[270px] truncate">{displayName}</span>
        </div>
        <span className="text-grey text-sm font-medium leading-5.5 w-[200px] truncate">{member.designation}</span>
      </div>
