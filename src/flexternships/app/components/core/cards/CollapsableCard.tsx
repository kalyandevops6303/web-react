import {
    ...
    AccordionItem,
    AccordionTrigger,
} from "@flexternships/components/ui/accordion"
            ...
            <AccordionItem value="item-1">
                <AccordionTrigger className={`${Styles.collapsableCard} hover:no-underline`}>
                    <div className="flex w-full items-center mr-3 justify-between">
                        ...
                        <div>
                            <Link to={link?.href || "#"} className="text-center text-[14px] font-semibold tracking-[0.4px] text-[#0185E4]">{link?.text}</Link>
                        </div>
                    </div>
                </AccordionTrigger>
                <AccordionContent data-state={isOpen}>
                    {children}
                </AccordionContent>
            </AccordionItem>
        </Accordion>
