    ...
    <div className={`${Styles.formFieldContainer} ${className ?? ''}`}>
      ...
      <div className={Styles.formNumberInputContainer}>
        <button
          ...
          className={`${Styles.inputAction} ${Styles.leftInputAction} ${
            min !== undefined && value <= min ? 'opacity-50 cursor-not-allowed' : ''
          }`}
        >
          <Minus size={16} />
        </button>

        <input type="text" readOnly value={value} className={Styles.formNumberInput} />
        <button
          onClick={handleIncrement}
          disabled={max !== undefined && value >= max}
          className={`${Styles.inputAction} ${Styles.rightInputAction} ${
            max !== undefined && value >= max ? 'opacity-50 cursor-not-allowed' : ''
          }`}
        >
          <Plus size={16} />
        </button>
