    ...
    return (
        <div className={`flex gap-4 ${className || ''}`}>
            <div className='h-[100px] w-[100px] rounded-full bg-white-fa'>
                {value ? (
                    <img 
                        src={value.startsWith('https') ? value : (fileInputRef.current?.files?.[0] ? URL.createObjectURL(fileInputRef.current.files[0]) : undefined)}
                        alt="Avatar" 
                    ...
                    <Popover>
                        ...
                        <PopoverContent className="p-0 border-none" align="start" sideOffset={-2}>
                            <div className='flex flex-col rounded-md bg-white border-1 border-solid border-grey-border'>
                                <button onClick={handleUploadClick} className='flex items-center gap-x-2 py-3 px-4 text-grey hover:bg-trublue-light hover:text-trublue-secondary-500'>
                                    <Upload size={18} />
                                    <span className='text-sm leading-5.5'>
                                        Upload from device
                                    </span>
                                </button>
                                <button onClick={handleRemoveClick} className='flex items-center gap-x-2 py-3 px-4 text-grey hover:bg-trublue-light hover:text-trublue-secondary-500'>
                                    <Trash2 size={18} />
