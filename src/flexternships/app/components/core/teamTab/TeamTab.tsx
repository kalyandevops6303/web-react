...
import { useFlexternProjectDetailsStore } from '@/flexternships/stores/project-details-store';

export default function TeamTab(props: InputProps) {
  const { className } = props;
  const { projectId } = useParams<{ projectId: string }>();
  const populateTeamDetails = useFlexternProjectDetailsStore((state) => state.populateTeamDetails);
  const teamDetails = useFlexternProjectDetailsStore((state) => state.teamDetails);
  useEffect(() => {
